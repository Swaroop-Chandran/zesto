...
<div class="admin-layout font-sans bg-zesto-dark text-[#dfe2eb] min-h-screen flex">
  ...
  <div class="flex-1 overflow-auto p-6 md:p-10 max-w-7xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-extrabold text-white">Coupons & Promotional Offers</h1>
        <p class="text-sm text-white/60 mt-1">Manage platform discount coupons and marketing homepage banner offers.</p>
      </div>
      <div class="flex gap-2">
        ...
        <button onclick="openOfferModal('add')" class="btn-secondary flex items-center gap-2 text-sm px-4 py-2 border border-zesto-orange/50 text-zesto-orange hover:bg-zesto-orange/20 rounded-xl font-bold transition-all glass-panel">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M12 4v16m8-8H4"/></svg>
          Add Banner Offer
        </button>
      </div>
    ...
    <div class="flex border-b border-white/10 mb-8 gap-6">
      <button onclick="switchTab('coupons')" id="tab-btn-coupons" class="tab-btn pb-4 font-bold text-sm border-b-2 border-zesto-orange/50 text-zesto-orange transition-all">
        Platform Coupons (<?= count($coupons) ?>)
      </button>
      <button onclick="switchTab('offers')" id="tab-btn-offers" class="tab-btn pb-4 font-bold text-sm text-white/60 hover:text-white/90 transition-all">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm text-left">
            <thead class="bg-white/5">
              <tr>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Coupon Code</th>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Discount</th>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Max Discount Limit</th>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Min Order Value</th>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Status</th>
                <th class="px-6 py-4 text-xs font-bold text-white/40 uppercase tracking-wider">Created</th>
            ...
            <tbody class="divide-y divide-white/5">
              ...
              <tr>
                <td colspan="7" class="px-6 py-10 text-center text-white/40 font-semibold">No coupons available. Click "Add Coupon" to create one.</td>
              </tr>
              <?php endif; ?>
              <?php foreach ($coupons as $cp): ?>
              <tr class="hover:bg-white/5 transition-colors">
                <td class="px-6 py-4">
                  <div class="inline-block px-3 py-1 font-mono font-bold text-sm bg-zesto-orange/20 text-zesto-orange rounded-lg tracking-wider border border-[#ffdbd0] hover:scale-105 transition-transform duration-200">
                    <?= e($cp['code']) ?>
                  </div>
                </td>
                <td class="px-6 py-4 font-extrabold text-white/90">
                  <?= (float)$cp['discount_percentage'] ?>% OFF
                </td>
                <td class="px-6 py-4 font-semibold text-white/80">
                ...
                <td class="px-6 py-4 text-xs text-white/40">
                  <?= date('M j, Y', strtotime($cp['created_at'])) ?>
                </td>
                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                  <button onclick="editCoupon(<?= htmlspecialchars(json_encode($cp), ENT_QUOTES, 'UTF-8') ?>)" class="text-zesto-orange hover:text-[#d24200] font-bold text-sm">Edit</button>
    ...
    <div id="tab-content-offers" class="tab-pane hidden">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <?php if (empty($offers)): ?>
        <div class="col-span-full glass-panel rounded-2xl border border-white/10 shadow-md shadow-black/20 p-10 text-center text-white/40 font-semibold">
          No promotional banner offers configured. Click "Add Banner Offer" to create one.
        </div>
        <?php endif; ?>
        <?php foreach ($offers as $of): ?>
        <div class="glass-panel rounded-2xl border border-white/10 p-5 shadow-md shadow-black/20 hover:shadow-md transition-all flex flex-col justify-between">
          <div>
            <!-- Banner Image Preview -->
            <?php if ($of['image']): ?>
            <div class="w-full h-40 rounded-xl overflow-hidden mb-4 bg-white/5 border border-white/10">
            ...
            <div class="w-full h-40 rounded-xl bg-gradient-to-r from-[#ffdbd0] to-[#ffeadb] flex items-center justify-center mb-4 border border-[#ffdbd0]">
              <span class="text-zesto-orange font-bold text-xl tracking-wide"><?= e($of['code'] ?? 'ZESTO PROMO') ?></span>
            </div>
            <?php endif; ?>

            <!-- Title & Description -->
            <div class="flex justify-between items-start gap-4 mb-2">
              <h3 class="font-extrabold text-lg text-white"><?= e($of['title']) ?></h3>
              <form method="POST" class="inline">
                <?= csrfField() ?>
            ...
            <p class="text-sm text-white/70 mb-4 font-medium"><?= e($of['description']) ?></p>
            
            <?php if ($of['code']): ?>
            <div class="mb-4">
              <span class="text-xs text-white/40 font-bold uppercase tracking-wider block mb-1">Linked Coupon</span>
              <span class="inline-block font-mono font-bold text-xs bg-white/10 text-white/80 px-3 py-1 rounded-md border border-white/10"><?= e($of['code']) ?></span>
            </div>
            <?php endif; ?>
          </div>
          ...
          <div class="flex justify-end gap-3 border-t border-white/10 pt-4 mt-2">
            <button onclick="editOffer(<?= htmlspecialchars(json_encode($of), ENT_QUOTES, 'UTF-8') ?>)" class="text-sm px-4 py-2 border border-white/10 rounded-xl font-bold hover:bg-white/5 transition-colors">
              Edit Offer
            </button>
...
<div id="coupon-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300">
  <div class="glass-panel rounded-2xl w-full max-w-md p-6 shadow-2xl relative translate-y-4 transition-transform duration-300">
    <button onclick="closeCouponModal()" class="absolute top-4 right-4 text-white/40 hover:text-white/70">
    ...
    <h2 id="coupon-modal-title" class="text-xl font-extrabold text-white mb-6">Add New Coupon</h2>
    ...
    <form method="POST" id="coupon-form">
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Coupon Code</label>
          <input type="text" name="code" id="coupon-code" placeholder="e.g. EXTRA50" required
                 class="w-full px-4 py-3 rounded-xl border border-white/10 focus:outline-none focus:border-zesto-orange/50 font-mono uppercase font-bold tracking-wider">
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Discount %</label>
          ...
          <div>
            <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Max Discount (₹)</label>
        ...
        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Min Order Value (₹)</label>
        ...
        <div class="flex items-center gap-3 pt-2">
          ...
          <label for="coupon-active" class="text-sm font-bold text-white/80">Set coupon as active immediately</label>
        </div>
      </div>

      <div class="mt-8 flex justify-end gap-3">
        <button type="button" onclick="closeCouponModal()" class="px-5 py-3 border border-white/10 text-white/60 rounded-xl font-bold hover:bg-white/5 transition-colors">
          Cancel
        </button>
...
<div id="offer-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300">
  <div class="glass-panel rounded-2xl w-full max-w-lg p-6 shadow-2xl relative translate-y-4 transition-transform duration-300">
    <button onclick="closeOfferModal()" class="absolute top-4 right-4 text-white/40 hover:text-white/70">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M6 18L18 6M6 6l12 12"/></svg>
    </button>
    
    <h2 id="offer-modal-title" class="text-xl font-extrabold text-white mb-6">Add Promotional Banner</h2>
    ...
    <form method="POST" id="offer-form" enctype="multipart/form-data">
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Offer Title</label>
          <input type="text" name="title" id="offer-title" placeholder="e.g. 50% Off First 3 Orders" required
                 class="w-full px-4 py-3 rounded-xl border border-white/10 focus:outline-none focus:border-zesto-orange/50 font-semibold text-white/80">
        </div>

        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Short Description</label>
          <textarea name="description" id="offer-desc" placeholder="Details about this discount or kitchen campaign..." rows="3"
                    class="w-full px-4 py-3 rounded-xl border border-white/10 focus:outline-none focus:border-zesto-orange/50 font-medium text-white/80 resize-none"></textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Linked Coupon Code (Optional)</label>
            <select name="code" id="offer-code"
                    class="w-full px-4 py-3 rounded-xl border border-white/10 focus:outline-none focus:border-zesto-orange/50 font-semibold text-white/80 glass-panel">
          ...
          <div>
            <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Or Custom Code Link</label>
            <input type="text" name="custom_code" id="offer-custom-code" placeholder="e.g. FESTIVE20"
                   class="w-full px-4 py-3 rounded-xl border border-white/10 focus:outline-none focus:border-zesto-orange/50 font-mono uppercase font-bold text-white/80">
          </div>
        </div>

        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Upload Banner Image File</label>
          ...
          <span class="text-[10px] text-white/40 font-semibold mt-1 block">Recommended size: 800x400 JPG, PNG or WEBP (Max 5MB)</span>
        </div>
        ...
        <div>
          <label class="block text-xs font-bold text-white/40 uppercase tracking-wider mb-2">Banner Image URL</label>
        ...
        <div class="flex items-center gap-3 pt-2">
          <input type="checkbox" name="is_active" id="offer-active" checked value="1"
                 class="w-5 h-5 accent-[#a83300] rounded focus:ring-0">
          <label for="offer-active" class="text-sm font-bold text-white/80">Display this banner immediately on home page</label>
        </div>
      </div>

      <div class="mt-8 flex justify-end gap-3">
        <button type="button" onclick="closeOfferModal()" class="px-5 py-3 border border-white/10 text-white/60 rounded-xl font-bold hover:bg-white/5 transition-colors">
          Cancel
        </button>
        <button type="submit" class="px-6 py-3 bg-[#a83300] text-white rounded-xl font-bold hover:bg-[#d24200] transition-colors">
          Save Banner
...
function switchTab(tab) {
  ...
  if (tab === 'coupons') {
    btnCoupons.className = "tab-btn pb-4 font-bold text-sm border-b-2 border-zesto-orange/50 text-zesto-orange transition-all";
    btnOffers.className = "tab-btn pb-4 font-bold text-sm text-white/60 hover:text-white/90 transition-all";
    paneCoupons.classList.remove('hidden');
    paneOffers.classList.add('hidden');
  } else {
    btnOffers.className = "tab-btn pb-4 font-bold text-sm border-b-2 border-zesto-orange/50 text-zesto-orange transition-all";
    btnCoupons.className = "tab-btn pb-4 font-bold text-sm text-white/60 hover:text-white/90 transition-all";
