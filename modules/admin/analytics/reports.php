...
<div class="admin-layout font-sans bg-zesto-dark text-[#dfe2eb] min-h-screen flex">
  ...
  <div class="flex-1 overflow-auto p-6 md:p-10 max-w-7xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
      <h1 class="text-2xl font-extrabold text-white">Reports &amp; Analytics</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
      ...
      <div class="glass-panel rounded-2xl border border-white/10 p-5 shadow-md shadow-black/20">
        ...
        <p class="text-xs text-white/40 font-bold uppercase tracking-wider mb-1"><?= $label ?></p>
    ...
    <div class="glass-panel rounded-2xl border border-white/10 shadow-md shadow-black/20 overflow-hidden">
      <div class="p-5 border-b border-white/10"><h3 class="font-bold text-sm text-white">Revenue by Restaurant</h3></div>
      <table id="rest-table" class="w-full text-sm">
        <thead class="bg-white/5">
          <tr>
            <th class="text-left px-4 py-3 text-xs font-bold text-white/40 uppercase">Restaurant</th>
            <th class="text-left px-4 py-3 text-xs font-bold text-white/40 uppercase">Orders</th>
            <th class="text-left px-4 py-3 text-xs font-bold text-white/40 uppercase">Revenue</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-white/5">
          ...
          <tr class="hover:bg-white/5">
            ...
            <td class="px-4 py-3 font-bold text-zesto-orange"><?= formatPrice($r['revenue']) ?></td>
          </tr>
          <?php endforeach; ?>
          <?php if (empty($restData)): ?>
          <tr><td colspan="3" class="px-4 py-8 text-center text-white/40">No data for selected period</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
