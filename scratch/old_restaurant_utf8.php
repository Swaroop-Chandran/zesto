<?php
/**
 * Zesto ΓÇö Swiggy-Style Restaurant Detail & Menu Page v2.0
 */
...
$pageTitle   = $restaurant['name'] . ' Menu ΓÇö Order on Zesto';
...
<main class="flex-1 bg-[#f5f5f0] pb-mobile-nav" id="restaurant-page">
  ...
  <!-- ΓöÇΓöÇ Back Button ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
  ...
  <!-- ΓöÇΓöÇ Restaurant Banner ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
  <div class="relative h-48 md:h-72 lg:h-80 bg-[#1b1c1c] overflow-hidden mt-3">
    <img src="<?= e(getRestaurantBanner($restaurant)) ?>"
         alt="<?= e($restaurant['name']) ?> ΓÇö Restaurant Banner"
         class="w-full h-full object-cover opacity-80">
    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
  </div>

  <!-- ΓöÇΓöÇ Restaurant Info Card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-3 mt-3">
            ...
            <div class="flex items-center gap-1.5 text-sm text-gray-600 font-semibold bg-gray-100 px-2.5 py-1 rounded-lg">
              ...
              <?= e($restaurant['delivery_time'] ?: '30ΓÇô45 min') ?>
            </div>
            
            <!-- Delivery Fee -->
            <div class="text-sm text-gray-600 font-semibold bg-gray-100 px-2.5 py-1 rounded-lg">
              <?= $restaurant['is_free_delivery'] ? '≡ƒÄë Free Delivery' : 'Γé╣' . number_format($restaurant['delivery_fee'], 0) . ' delivery' ?>
            </div>
  ...
  <!-- ΓöÇΓöÇ Menu Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
  <div class="max-w-[1280px] mx-auto px-4 md:px-8 lg:px-10 mt-6 md:mt-8">
    ...
    <div class="flex gap-6 lg:gap-8 items-start">
      ...
      <!-- Category Nav ΓÇö desktop sticky sidebar -->
      ...
      <nav class="hidden lg:block w-52 shrink-0 sticky top-4 bg-white rounded-2xl border border-[#ece9e6] p-3 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-none">
        ...
        <a href="#section-recommended" onclick="scrollToSection('recommended')" class="category-nav-item active" data-section="recommended">
          Γ¡É Recommended (<?= count($recommended) ?>)
        </a>
        <?php endif; ?>
      ...
      <div class="flex-1 min-w-0 space-y-8" id="menu-container">
        ...
        <div class="menu-section" data-section="recommended" id="section-recommended">
          <div class="menu-section-divider">
            <h3>Γ¡É Recommended <span class="text-gray-400 font-medium text-sm"><?= count($recommended) ?> items</span></h3>
          </div>
        ...
        <div id="menu-empty" class="hidden py-16 text-center">
          <div class="text-4xl mb-3">≡ƒöì</div>
  ...
  <!-- ΓöÇΓöÇ Customer Reviews & Feedback Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ -->
    ...
    <div class="bg-white rounded-2xl md:rounded-3xl border border-[#ece9e6] p-6 md:p-8 shadow-sm flex flex-col gap-6 font-sans">
      <div class="flex justify-between items-center border-b border-gray-100 pb-4">
        ...
        <div class="text-right">
          <p class="text-2xl font-black text-green-600 flex items-center gap-1">Γÿà <?= number_format((float)($restaurant['rating'] ?? 0), 1) ?></p>
      ...
      <div class="text-center py-10 text-gray-400 font-bold text-sm">
        ≡ƒì│ No customer reviews left for this kitchen yet. Be the first to order and review!
      </div>
      <?php else: ?>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-gray-50 rounded-2xl border border-gray-100 p-4 flex flex-col gap-1.5 text-xs font-semibold text-gray-700">
          <div class="flex justify-between items-center font-extrabold">
            ...
            <span class="text-amber-500 font-bold">Γÿà <?= number_format((float)($rev['restaurant_rating'] ?? 0), 1) ?> rating</span>
          </div>
          <p class="text-gray-600 leading-normal">"<?= e($rev['review_text']) ?>"</p>
          <span class="text-[9px] text-gray-400 font-bold uppercase tracking-wider block mt-1"><?= date('M j, Y ΓÇö g:i A', strtotime($rev['created_at'])) ?></span>
        </div>
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
...
<script>
// ΓöÇΓöÇ Cart quantities state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const cartQty = {};
...
async function cartAdd(itemId, restaurantId, restaurantSlug) {
  ...
  try {
    ...
    if (data.success) {
      ...
      Zesto.toast('≡ƒ¢Æ Added to cart!', 'cart');
    } else {
...
function renderStepper(itemId, restaurantId, restaurantSlug) {
  ...
  wrap.innerHTML = `
    <div class="qty-stepper">
      <button class="qty-stepper-btn" onclick="cartDecrement(${itemId},${restaurantId},'${restaurantSlug}')">ΓêÆ</button>
...
// ΓöÇΓöÇ Menu search & veg filter ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
let vegOnly = false;
let searchQ  = '';
...
// ΓöÇΓöÇ Category nav active state (scroll spy) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
